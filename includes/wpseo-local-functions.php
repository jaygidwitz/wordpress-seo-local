...
function wpseo_local_show_address( $atts ) {
	...
	if ( $atts['show_phone'] && ! empty( $business_phone ) ) {
		...
		$output .= '<span class="wpseo-phone">' . __( 'Phone', 'yoast-local-seo' ) . ': <a href="tel:' . $href_business_phone . '" class="tel" itemprop="telephone">' . $business_phone . '</a></span>' . ( $atts['oneline'] ? ' ' : '<br/>' );
	}

	if ( $atts['show_phone_2'] && ! empty( $business_phone_2nd ) ) {
		$href_business_phone_2nd = preg_replace( "/[^0-9,.]/", "", $business_phone_2nd );
		$output .= '<span class="wpseo-phone2nd">' . __( 'Secondary phone', 'yoast-local-seo' ) . ': <a href="tel:' . $href_business_phone_2nd . '" class="tel">' . $business_phone_2nd . '</a></span>' . ( $atts['oneline'] ? ' ' : '<br/>' );
	}
